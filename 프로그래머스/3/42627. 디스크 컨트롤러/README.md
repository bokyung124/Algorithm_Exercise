...
1. 작업을 요청 시점 순으로 정렬
2. 현재 시간보다 이전에 요청된 모든 작업들을 최소 힙에 추가
  - `(작업 소요 시간, 작업 요청 시점)` 형태로 추가
3. 힙에서 가장 작은 작업을 꺼내서 처리
4. 현재 시간 업데이트
  - 작업을 처리한 경우 -> 현재 시간 = 현재 시간 + 작업 소요 시간
  - 작업을 처리하지 못한 경우 -> 현재 시간 = 다음 작업의 요청 시간
5. 총 소요 시간에 현재 시간 - 작업 요청 시간 더해줌
6. 모든 작업을 처리할 때까지 2 - 5 반복
